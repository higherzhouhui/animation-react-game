...
import GameClose from "../../components/GameClose/index";
...
const XocDia = () => {
  ...
  const handleBet = async (index: number) => {
    if (!betAble) return;
    if (Number(userInfo.goldCoin) < money) return Toast.show(t("yu_e_bu_zu"));
      ...
      {loading ? (
        ...
        <div className={style.table}>
          ...
          <div className={style.tabelGrid} id="tabelGrid" style={{ width: tabelGrid.width > 0 ? tabelGrid.width : "100%", height: tabelGrid.height > 0 ? tabelGrid.height : "100%" }}>
            <img
              src={require("@/assets/image/tz/background/game-table.png")}
              alt=""
            ...
            <div
              ...
              style={{
                ...
                height: `${tabelGrid.height * 0.72}px`,
              }}>
                ...
                return (
                  <div key={index} className={`${style.box} ${s.includes(index) ? style.selected : ""} tabelArea0`} onClick={() => handleBet(index)}>
                    ...
                    <div className={`${style.text} ${!item.ball ? style.odds1 : ""}`} style={index > 3 ? { left: "50%", transform: "translate(-50%,-50%)" } : { right: `${tabelGrid.width * 0.05}px` }}>
                      {item.name && (
                        <div className={`${style.disFlexs} ${style[item.type]}`}>
                          {item.name.map((val: any, index: number) => {
                            return <Icon size={14} num={val} key={index} className={style.icon} />;
                          })}
                        </div>
                      )}
                      {item.odds}
          ...
          {/* 筹码盘 */}
          <Chip grid={"tabelArea0"} offsetTop={(document.documentElement.clientHeight - tabelGrid.height) / 2} ref={ChipRef} downTime={downTime} moneySelectList={moneySelectList} type={lotteryName} chipPoint={lastRecord.lotteryResult} gameItem={gameArray} lotteryName="toubao" betAble={betAble} betAbleSet={betAbleSet} expect={issueData.expect} hasVer />
          {/* 游戏规则 */}
