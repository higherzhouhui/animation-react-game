...
const RecingAni = (props: { carResult: string[] }) => {
  const noTrans = document.documentElement.clientWidth < document.documentElement.clientHeight;
  ...
  return (
    <div className={`${style.aniBody} ${noTrans ? style.aniBody1 : ""}`}>
      {showPlatform ? (
          ...
          <div className={`${style.block} ${showTer ? style.showTer : ""}`}>
            ...
            {startTime > 0 ? (
              ...
              <div className={style.carList}>
                {carList.map((val) => {
                  return (
                    <div className={`icon-dice-car${val.name} ${style.car}`} style={{ transform: `translate3d(${((-1 * val.offsetX) / 100) * (noTrans ? window.innerHeight : window.innerWidth - 50)}px,0,0)` }} key={val.name}>
