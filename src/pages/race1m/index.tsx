...
import RecingAni from "./recingAni";
...
const XocDia = () => {
  ...
  const { loading, dom: Load } = LoadingGame("tz", () => {
    init();
  });
  ...
  const [showAni, showAniSet] = useState(false); //展示赛车动画
  const [saicheResult, saicheResultSet] = useState<string[]>([]);
    ...
    try {
      ...
      Promise.all([setDelay()]).then((e) => {
        getLotteryResultHistoryByName<ILotteryHistory[]>().then((last) => {
          ...
          saicheResultSet(result.lotteryResult);
          if (isAni) {
            setTimeout(() => {
  ...
  const handleBet = async (index: number) => {
    ...
    let { num, lname, type, type_text } = gameArray.tabel[index];
    try {
      ...
      {loading ? (
        ...
        <div className={style.table}>
          ...
          <div className={style.tabelGrid} id="tabelGrid" style={{ width: tabelGrid.width > 0 ? tabelGrid.width : "100%", height: tabelGrid.height > 0 ? tabelGrid.height : "100%" }}>
            <img
              src={require("@/assets/image/race1m/background/game-table.png")}
              alt=""
            ...
            <div
              ...
              style={{
                width: `${tabelGrid.width * 0.84}px`,
                height: `${tabelGrid.height * 0.67}px`,
              }}>
              {gameArray.tabel.map((item, index) => {
                ...
                return (
                  <div key={index} className={`${style.box} ${s.includes(index) ? style.selected : ""} tabelArea0`} onClick={() => handleBet(index)}>
                    ...
                    <div className={`${style.text} ${!item.ball ? style.odds1 : ""}`} style={index > 3 ? { left: "50%", transform: "translate(-50%,-50%)" } : { right: `${tabelGrid.width * 0.05}px` }}>
                      {item.name && (
                        <div className={`${style.disFlexs}`}>
                          <Icon size={24} num={item.name} key={index} className={style.icon} />
                        </div>
                      )}
                      {item.odds}
                    </div>
          ...
          {/* 筹码盘 */}
          <Chip grid={"tabelArea0"} offsetTop={(document.documentElement.clientHeight - tabelGrid.height) / 2} ref={ChipRef} downTime={downTime} moneySelectList={moneySelectList} type={lotteryName} chipPoint={lastRecord.lotteryResult} gameItem={gameArray} lotteryName="saiche" betAble={betAble} betAbleSet={betAbleSet} expect={issueData.expect} hasVer showAniSet={showAniSet} />
          {/* 游戏规则 */}
          ...
          {showAni && <RecingAni carResult={saicheResult} />}
        </div>
      )}
    </div>
  );
