...
const Yuxx = () => {
  ...
  const handleBet = async (index: number) => {
    ...
    let { num, lname, type, type_text } = gameArray[0].tabel[index];
    try {
    ...
    <div id="gameBody" className={style.gameBody}>
      {loading ? (
        ...
        <div className={style.table}>
          ...
          <div className={style.switchBtn} onClick={() => betAble && gameActiveTabSet((e) => Math.abs(e - 1))}>
            {t("switchGame")}
          </div>
          {/* 桌子布局 */}
          {gameArray.map((it, kk) => {
              ...
              <div key={kk}>
                {gameActiveTab === kk && (
                  <div className={style.tabelGrid} id="tabelGrid" style={{ width: tabelGrid.width > 0 ? tabelGrid.width : "100%", height: tabelGrid.height > 0 ? tabelGrid.height : "100%" }}>
                    <img
                      src={require("@/assets/image/yuxx/background/game-table.png")}
                      alt=""
                    ...
                    />
                    <div
                      className={`${style.grid} tableGrid ${style["grid" + gameActiveTab]}`}
                      style={{
                      ...
                      {it.tabel.map((item, index) => {
                        ...
                        return (
                          <div key={index} className={`${style.box} ${s.includes(index) ? style.selected : ""} tabelArea${kk}`} onClick={() => handleBet(index)}>
                            {item.ball && <Ball list={item.ball} className={style.ballGrid} />}
                            <div className={`${style.text} ${!item.ball ? style.odds1 : ""}`}>
                              {item.name && (
                                <div className={style.disFlex}>
                                  {item.name.map((val: any, index: number) => {
                                    return <YuxxIcon num={val} key={index} />;
                                  })}
                                </div>
                              )}
                              {item.odds}
                ...
                {/* 筹码盘 */}
                {gameActiveTab === kk && <Chip grid={"tabelArea" + kk} offsetTop={(document.documentElement.clientHeight - tabelGrid.height) / 2} ref={ChipRef} downTime={downTime} moneySelectList={moneySelectList} type="yuxx" chipPoint={lastRecord.lotteryResult} gameItem={it} lotteryName={lotteryName} betAble={betAble} betAbleSet={betAbleSet} expect={issueData.expect} hasVer />}
              </div>
            );
          })}
          {/* 游戏规则 */}
