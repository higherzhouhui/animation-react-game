      ...
      {loading ? (
        ...
        <div className={style.table}>
          ...
          <div className={style.tabelGrid} id="tabelGrid" style={{ width: tabelGrid.width > 0 ? tabelGrid.width : "100%", height: tabelGrid.height > 0 ? tabelGrid.height : "100%" }}>
            <img
              src={require("@/assets/image/jsks/background/game-table.png")}
              alt=""
            ...
            <div
              ...
              style={{
                ...
                marginTop: `${tabelGrid.height * -0.02}px`,
              }}>
                      ...
                      {item.name && (
                        <div className={style.disFlexs}>
                          {item.name.map((val: any, index: number) => {
                            return <Icon size={item.name?.length > 1 ? 18 : 24} num={val} key={index} />;
                          })}
                        </div>
                      )}
                      {item.odds}
          ...
          {/* 筹码盘 */}
          <Chip grid={"tabelArea0"} offsetTop={(document.documentElement.clientHeight - tabelGrid.height) / 2} ref={ChipRef} downTime={downTime} moneySelectList={moneySelectList} type="jsks" chipPoint={lastRecord.lotteryResult} gameItem={gameArray} lotteryName={lotteryName} betAble={betAble} betAbleSet={betAbleSet} expect={issueData.expect} hasVer />
          {/* 游戏规则 */}
